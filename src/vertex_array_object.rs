...
impl VertexArrayObject {
    ...
    fn check_mega_buffer_object_data_id(&self, mega_buffer_object_data: &MegaBufferObjectData) -> Result<(), String> {
        if self.mega_buffer_object_data_id != mega_buffer_object_data.id {
            return Err(String::from(
                "The MegaBufferObjectDataId registered in the VertexArrayObject does not match \
                 the MegaBufferObjectDataId passed as an argument."
            ));
        }
        Ok(())
    }
    ...
    pub fn add_vertex_buffer_object_attribute(
        ...
        let sub_buffer = mega_buffer_object_data.sub_buffers
            .get(buffer_object_index)
            .ok_or_else(|| format!(
                "SubBuffer index out of bounds: specified {}, but max len is {}.",
                buffer_object_index,
    ...
    pub fn connect_index_buffer_object(
        &self,
        mega_buffer_object_data: &MegaBufferObjectData,
    ) -> Result<(), String> {
        self.check_mega_buffer_object_data_id(mega_buffer_object_data)?;
